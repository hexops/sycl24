...
fn zigAlloc(sz: usize, user_data: ?*const anyopaque) callconv(.C) ?*anyopaque {
    var allocator: *const std.mem.Allocator = @ptrCast(@alignCast(user_data));
...
fn zigFree(ptr: ?*anyopaque, user_data: ?*const anyopaque) callconv(.C) void {
    var allocator: *const std.mem.Allocator = @ptrCast(@alignCast(user_data));
